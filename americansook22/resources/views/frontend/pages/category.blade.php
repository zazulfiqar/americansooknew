...
<div class="ourProSec">
  ...
  <h2>
    @if(isset($session))
    @php
      echo $data_cat_title = GoogleTranslate::trans("$catname->title",Session::get('lang'));
    @endphp
@else
$catname->title
@endif
  ...
  <div class="container">
    <div class="row">
        @foreach ($categoryname as $allsubcat)
        <div class="col-md-2 col-sm-2 col-xs-12">
            <div class="product">
                {{-- {{route('product-detail',$allcat->id)}} --}}
                <a href="{{route('subCategory',$allsubcat->id)}}">
            <div class="proImg">
                <img src="{{url('/americansook/public/../storage/app/'.$allsubcat->photo)}}" class="img-responsive" alt="proImg">
            </div>
            </a>
            <div class="proCntnt">
                <h6>
                  @if(isset($session))
                  @php
                    echo $data_cat_title = GoogleTranslate::trans("$allsubcat->title",Session::get('lang'));
                  @endphp
              @else
              $allsubcat->title
              @endif
